...
/************************************************************************
	GG Modified to support the two Sid chips
...
************************************************************************/
void setwaveform_triangleSid1(uint8_t channel)
{
  ...
  {
     mySid.set_registerSid1(channel+dataset[n], dataset[n+1]); 
     mySid.set_registerSid1(channel+dataset[n], dataset[n+1]); 
...
void setwaveform_noiseSid2(uint8_t channel)
{
  ...
  while(dataset[n]!=0xFF) 
  {
     mySid.set_registerSid2(channel+dataset[n], dataset[n+1]); 
     mySid.set_registerSid2(channel+dataset[n], dataset[n+1]); 
     // register address, register content
     n+=2;
  }
...
************************************************************************/
// pitch=16.77*frequency
void set_frequency(uint8_t sid, uint16_t pitch,uint8_t channel)
{
  if (sid==1) {
    mySid.set_registerSid1(channel, pitch&0xFF); // low register adress
    mySid.set_registerSid1(channel+1, pitch>>8); // high register adress
  }else {
    mySid.set_registerSid2(channel, pitch&0xFF); // low register adress
    mySid.set_registerSid2(channel+1, pitch>>8); // high register adress
  }
}
...
void loop()
{
  ...
  setwaveform_triangleSid1(CHANNEL1);
  setwaveform_triangleSid1(CHANNEL2);

  setwaveform_noiseSid2(CHANNEL1);
  setwaveform_noiseSid2(CHANNEL2);

  // Double Roll: Sid1 and then Sid2  
  int c=50; // 5 secs

  // Mute All:
  set_frequency(1,0,CHANNEL1);
  set_frequency(1,0,CHANNEL2);
  set_frequency(2,0,CHANNEL1);
  set_frequency(2,0,CHANNEL2);
  
  while(c-- >0)
  {    
    n=zufall()*8;
    set_frequency(1,n*17,CHANNEL1); // change noise generator frequency
    n=zufall()*2;
    set_frequency(1,n*17,CHANNEL2); // change noise generator frequency
  ...
  {    
    n=zufall()*8;
    set_frequency(2,n*17,CHANNEL1); // change noise generator frequency
    n=zufall()*2;
    set_frequency(2,n*17,CHANNEL2); // change noise generator frequency
